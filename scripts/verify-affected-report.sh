...
jq -e '.scheduling.objective == ["predictedRuntimeMakespanMs","totalCheckoutPathCount","targetBucketRuntimeMs","assignmentId"] and .scheduling.totalCheckoutPathCount >= .scheduling.uniqueCheckoutPathCount and .scheduling.duplicatedCheckoutPathCount == (.scheduling.totalCheckoutPathCount - .scheduling.uniqueCheckoutPathCount) and .scheduling.predictionSources.cold > 0' <<<"$RESULT" >/dev/null
