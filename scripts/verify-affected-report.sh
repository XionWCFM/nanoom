...
jq -e '.scheduling.objective == ["predictedRuntimeMakespanMs","totalCheckoutPathCount","targetBucketRuntimeMs","assignmentId"] and .scheduling.totalCheckoutPathCount >= .scheduling.uniqueCheckoutPathCount and .scheduling.duplicatedCheckoutPathCount == (.scheduling.totalCheckoutPathCount - .scheduling.uniqueCheckoutPathCount) and ((.scheduling.historyStatus == "loaded" and .scheduling.predictionSources.sampleCount > 0 and (.scheduling.predictionSources.exact + .scheduling.predictionSources.group) > 0) or (.scheduling.historyStatus == "bootstrap-fallback" and .scheduling.predictionSources.cold > 0))' <<<"$RESULT" >/dev/null
