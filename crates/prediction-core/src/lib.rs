...
impl PredictionContext {
    ...
    pub fn candidates(&self, group: &str, task_runner: &str, environment: &str) -> Vec<Scope> {
...
impl PredictionIndex {
    pub fn new(
        ...
        Self::new_with_tables(bundles, [])
    }

    pub fn new_with_tables(
        bundles: impl IntoIterator<Item = PredictionArtifactBundle>,
        compact_tables: impl IntoIterator<Item = PredictionTable>,
    ) -> Result<Self, String> {
        ...
        for table in compact_tables {
            table.validate()?;
            let scope_id = table.scope.id()?;
            if tables.insert(scope_id, table).is_some() {
                return Err("duplicate PredictionTable scope".into());
            }
        }
        Ok(Self { tables })
    ...
    pub fn from_tables(tables: impl IntoIterator<Item = PredictionTable>) -> Result<Self, String> {
        Self::new_with_tables([], tables)
    }
...
impl PredictionTable {
    pub fn load(path: &std::path::Path) -> Result<Self, String> {
        let bytes = read_bounded(path, MAX_PREDICTION_BYTES)?;
        let table: Self = serde_json::from_slice(&bytes).map_err(|error| error.to_string())?;
        table.validate()?;
        Ok(table)
    }
