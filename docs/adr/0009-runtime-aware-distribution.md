...
- workspace manifest의 `packages/*`에서 `*`는 경로 구분자를 넘지 않고, `.gitignore`로 제외된 `.next` 같은 생성 산출물의 manifest를 workspace로 다시 발견하지 않는다.
...
- `history`: 성공 sample과 기존 history를 병합해 key별 최근 7개만 남긴다. Timing sample 이름은 GitHub job과 assignment ID를 함께 포함해 서로 다른 job의 동일 assignment ID가 충돌하지 않게 한다.
