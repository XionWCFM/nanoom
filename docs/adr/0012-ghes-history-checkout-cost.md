...
- 기본 `run`과 `history`는 GitHub.com용 `actions/upload-artifact@v4.6.2`를 사용한다. GHES 사용자는 같은 shell 구현과 입출력 계약을 공유하는 `run-ghes`와 `history-ghes`로 Node 24 백포트 `v3.2.2`를 선택한다. composite Action은 `uses:`를 파라미터화할 수 없고 false인 조건부 `uses:`도 사전 다운로드하므로 단일 input 분기는 사용할 수 없다. 서버는 자동 감지하지 않으며 prefix fan-in은 공통 GitHub REST API로 구현한다.
